<template>
  <div>
      ...
      <a-row :gutter="24">
        <a-col :span="8" :offset="1">
          <a-form-item
            ...
            :label="`title`"
          >
            <a-input v-model:value="formState[`title`]" placeholder="placeholder"></a-input>
          </a-form-item>
        </a-col>
        <a-col :span="8" :offset="1">
          <a-form-item
            ...
            :label="`Author`"
          >
            <a-input v-model:value="formState[`author`]" placeholder="placeholder"></a-input>
          </a-form-item>
        </a-col>
      </a-row>
      <a-row>
    ...
    <a-layout>
      <a-table :columns="columns" :data-source="posts.data" @change="onTableChange"
        ...
        <template #bodyCell="{ column, text, record }">
          ...
          <template v-else-if="column.dataIndex === 'operation'">
            <a-space :size="15"  style="text-align: center;">
              <router-link :to="{ name: 'post_update', params: { id: record.id }}">Edit</router-link>
              <a-popconfirm
...
export default defineComponent({
  setup() {
    ...
    const onDelete = (id) => {
      store.dispatch("post/delete", id).then(res => {
        store.dispatch("post/search", formState);
      })
    }
